...
- **EXPORT**: dos botones. «CSV (vista filtrada)» descarga los contratos que pasan los filtros actuales. «Base completa (CSV)» descarga **todo el dataset** (~120.000 contratos) paginando la API SODA directamente en el navegador — así la base completa nunca vive en el repo (GitHub rechaza >100 MB) pero siempre está a un clic, fresca desde la fuente. Ambos usan separador `;` y BOM UTF-8 (Excel), e incluyen `NIT entidad` y `NIT/doc contratista` para análisis de entidades en doble rol (p. ej. ITM como contratista del Distrito y como entidad que subcontrata).
